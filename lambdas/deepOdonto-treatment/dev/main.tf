terraform {
  backend "s3" {
    bucket = "deep-odonto-140023362908-terraform"
    region = "us-east-2"
    key    = "lambdas/deepOdonto-treatment/dev"
  }
}
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:dynamodb:${var.aws_region}:${var.aws_account}:table/deepOdonto-treatment-${var.aws_env}"
      },
      {
        "Effect": "Allow",
